...
?>
		...
		<script>
			...
			function open_url_in_new_tab() {
				var url = window.location.protocol + "//" + window.location.host + window.location.pathname +
					'?partition=alpha&experiment_name=small_test_experiment&reservation=&account=&mem_gb=1&time=60&worker_timeout=60&max_eval=5&num_parallel_jobs=5&gpus=1&num_random_steps=2&follow=1&live_share=1&send_anonymized_usage_stats=1&constraints=&result_names=RESULT%3Dmin&run_program=echo "RESULT%3A %25(x)%25(y)"&run_program_once=&cpus_per_task=1&nodes_per_job=1&seed=&dryrun=0&debug=0&revert_to_random_when_seemingly_exhausted=1&gridsearch=0&model=BOTORCH_MODULAR&external_generator=&n_estimators_randomforest=100&installation_method=pip&run_mode=local&disable_tqdm=0&verbose_tqdm=0&force_local_execution=0&auto_exclude_defective_hosts=0&show_sixel_general=0&show_sixel_trial_index_result=0&show_sixel_scatter=0&show_worker_percentage_table_at_end=0&occ=0&occ_type=euclid&no_sleep=0&slurm_use_srun=0&verbose_break_run_search_table=0&abbreviate_job_names=0&main_process_gb=8&max_nr_of_zero_results=50&slurm_signal_delay_s=0&max_failed_jobs=0&exclude=&username=&generation_strategy=&root_venv_dir=&dependency=&workdir=&dont_jit_compile=0&fit_out_of_design=0&refit_on_cv=0&show_generate_time_table=0&dont_warm_start_refitting=0&max_attempts_for_generation=20&num_restarts=20&raw_samples=1024&max_abandoned_retrial=20&max_num_of_parallel_sruns=16&number_of_generators=1&force_choice_for_ranges=0&no_transform_inputs=0&fit_abandoned=0&no_normalize_y=0&verbose=0&generate_all_jobs_at_once=1&save_to_database=0&flame_graph=0&checkout_to_latest_tested_version=0&parameter_0_name=x&parameter_0_type=range&parameter_0_min=123&parameter_0_max=1000&parameter_0_number_type=int&parameter_0_log_scale=false&parameter_1_name=y&parameter_1_type=range&parameter_1_min=5431&parameter_1_max=1234&parameter_1_number_type=float&parameter_1_log_scale=false&partition=alpha&num_parameters=2'
				window.open(url, '_blank');
			}
	...
	<body>
		<div id="scads_bar" class="header-container">
			<div class="header-logo-group">
				<a href="index">
					<img id='oologo' class="logo-img" src="new_logo.svg" alt="OmniOpt2-Logo">
				</a>
