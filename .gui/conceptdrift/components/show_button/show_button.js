...
function stopLoadingAnimation(button) {
  ...
  if (textSpan) {
    if (textSpan.hasAttribute('data-original-text')) {
      ...
      // Fallback: if no original text stored, set to "Show"
...
function handleLoadedContent() {
  ...
  $loaded.find('button').not('.show-button').addClass('btn btn-primary');
  ...
  // Fallback: Always stop loading animation after content is loaded
  // This ensures the button returns to "Show" even if no scripts were executed
  setTimeout(() => {
    if (window.activeButton) {
      stopLoadingAnimation(window.activeButton);
      console.log('Loading animation stopped - fallback after content loaded');
    }
  }, 500);
}
...
function setupShowButtonListeners() {
  ...
  // Note: Ensemble button is handled by script.js using iframe approach
  // Do not add event listener here to avoid conflicts
