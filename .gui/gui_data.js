...
var model_data = [
	...
	{
		id: 'TPE',
		name: 'TPE: Tree Parzen Estimator (hyperopt). Supports continuation.',
...
var hiddenTableData = [
	{
		...
		required: false,
		info: "Use Variable names like this: <br><code style=\"white-space: pre\" class=\"highlight_me dark_code_bg invert_in_dark_mode\">bash /absolute/path/to/run.sh --lr=%(lr) --epochs=%(epochs)</code>. See <a target=\"_blank\" href=\"tutorials.php?tutorial=run_program_once\">this tutorial</a> to learn about the <code>install.sh</code>-file",
	...
	{
		label: "Slurm-Dependencies",
		id: "dependency",
		type: "text",
		value: "",
		info: "A string like afterok:<job_id>, afternotok:<job_id>, after:<job_id>, afterany:<job_id>, singleton or omniopt_singleton, while singleton allows only one slurm job of that job to be running and omniopt_singleton uses all existing OmniOpt2 jobs as a list that needs to be finished before this job starts. Multiple dependencies can be separated by commas.",
		required: false,
		regex: "^((afterok:[0-9]+|afternotok:[0-9]+|after:[0-9]+|afterany:[0-9]+|omniopt_singleton|singleton)(,(afterok:[0-9]+|afternotok:[0-9]+|after:[0-9]+|afterany:[0-9]+|omniopt_singleton|singleton))*)*$",
	...
	},
	{
		...
		type: "textarea",
		value: "",
		placeholder: "e.g. f(x, y) = x**2 + 2*x*y + y**2",
		help: "An optional math formula that defines the objective. When set, OmniOpt2 will auto-generate a run-program and derive hyperparameter suggestions. Use LaTeX (\\\\sigma, \\\\sum, \\\\frac) or Python-style infix (sin(x), 2*x + y). Switching to this field hides the run_program textarea; exactly one of the two must be filled."
	},
	{
		label: "Formula mode",
		id: "formula_mode",
