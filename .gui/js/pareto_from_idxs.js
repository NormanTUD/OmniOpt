...
function load_pareto_graph_from_idxs () {
	...
	renderHeatmapCorrelations(table);
}

function renderHeatmapCorrelations(data) {
	try {
		let containerId = "pareto_heatmap_corr_container";
		...
		if (!container) {
			...
			document.getElementById("pareto_from_idxs_table").appendChild(container);
		}
		container.innerHTML = "";
		...
		if (!allEntries.length) {
			container.innerHTML = "<p>Keine Daten für Heatmap verfügbar.</p>";
			return;
		}
		...
		for (let i = 0; i < allEntries.length; i++) {
			...
			dimNames.forEach(dim => {
				let val = null;
				if (dim in entry.values) {
					val = entry.values[dim][0];
		...
		function pearson(x, y) {
			...
			let denominator = Math.sqrt((count * sumX2 - sumX * sumX) * (count * sumY2 - sumY * sumY));
		...
		let layout = {
			title: "Korrelations-Heatmap (Parameter & Ergebnisse)",
			autosize: true,
			margin: { t: 50, l: 150 },
			xaxis: {
				tickangle: -45
		...
		Plotly.newPlot(containerId, [trace], add_default_layout_data(layout, 1));
	} catch (e) {
