#!/usr/bin/env python3
"""Slurm master container entrypoint (Python replacement for the bash
``docker-entrypoint.sh``).  Sets ``SLURM_CPUS_ON_NODE``, patches the
``slurm.conf`` template and starts the slurmctld plus ssh.
"""
