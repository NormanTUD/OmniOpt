...
function add_load_worker_cpu_button () {
	if (!$("#load_worker_cpu_button").length) {
...
function add_load_log_button () {
	if (!$("#load_debug_log_button").length) {
...
async function plot_worker_cpu_ram() {
	...
	Object.entries(hostData).forEach(([hostname, { timestamps, cpuUsage, ramUsage }], index) => {
		...
		showSpinnerOverlay(`Loading worker CPU/RAM plot: Plotting ${hostname} (${i} of ${Object.entries(hostData).length})`);
