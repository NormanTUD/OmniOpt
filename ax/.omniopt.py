...
def run_random_search(random_steps, ax_client, executor):
    ...
    print(f"random_steps {random_steps} + rand_in_prev_job {rand_in_prev_job} >= count_done_jobs() {count_done_jobs()} - nr_inserted_jobs {nr_inserted_jobs} + 1 and not search_space_exhausted {search_space_exhausted}")
