...
Original file is located at
    https://colab.research.google.com/drive/1DVhervZnA5evVTjneInrEOG1mTHzHXb-
"""
...
def wrapper_print_debug(func):
    def wrapper(*__args, **kwargs):
        ...
        result = func(*__args, **kwargs)  # result could now be a single float or a list of floats
        end_time = time.time()

        runtime = end_time - start_time
        runtime_human_readable = f"{runtime:.4f} seconds"

        # If runtime is significant, log it
        if runtime > 1:
            print_debug(f"@wrapper_print_debug: {func.__name__}(), runtime: {runtime_human_readable}")

        # Determine if `result` contains multiple values and format accordingly
        if isinstance(result, dict) and 'result' in result:
            if isinstance(result['result'], list):  # Handle multiple results
                print_debug(f"@wrapper_print_debug: {func.__name__}(), results: {result['result']}")
            else:  # Handle a single result
                print_debug(f"@wrapper_print_debug: {func.__name__}(), result: {result['result']}")
        else:
            print_debug(f"@wrapper_print_debug: {func.__name__}(), result: {result}")
...
def get_result(input_string):
    ...
    try:
        ...
        if matches:
            return [float(match) for match in matches]  # Return list of results as floats
        return None
...
def evaluate(parameters):
    ...
    return_in_case_of_error = {"results": [VAL_IF_NOTHING_FOUND]}
    ...
    try:
        ...
        # Ensure `result` is a list and handle the case of no results
        if result is None or len(result) == 0:
            result = [VAL_IF_NOTHING_FOUND]

        original_print(f"Result(s): {result}")
        ...
        headline = ["start_time", "end_time", "run_time", "program_string", *parameters_keys, "results", "exit_code", "signal", "hostname", *extra_vars_names]
        values = [start_time, end_time, run_time, program_string_with_params, *parameters_values, ",".join(map(str, result)), exit_code, _signal, socket.gethostname(), *extra_vars_values]
        ...
        # Return all results as a list in the dictionary
        if isinstance(result, list) and all(isinstance(r, (int, float)) for r in result):
            return {"results": [float(r) for r in result]}
