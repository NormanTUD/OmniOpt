...
# TEST_OUTPUT_MUST_CONTAIN: Average Result
# TEST_OUTPUT_MUST_CONTAIN: Average Results by Generation Method
...
def plot_graph(dataframe, save_to_file=None):
    ...
    plt.figure(figsize=(10, 6))
    sns.histplot(data=dataframe, x='result', hue='generation_method', multiple="stack", kde=False)
    plt.title('Distribution of Results by Generation Method')
    plt.xlabel('Result')
    ...
    plt.figure(figsize=(10, 6))
    sns.violinplot(x='generation_method', y='result', data=dataframe)
    plt.title('Violin Plot of Results by Generation Method')
    plt.xlabel('Generation Method')
    plt.ylabel('Result')
    plt.show()

    plt.figure(figsize=(10, 6))
    sns.stripplot(x='generation_method', y='result', hue='trial_status', data=dataframe, jitter=True)
    plt.title('Strip Plot of Results by Generation Method and Trial Status')
    plt.xlabel('Generation Method')
    plt.ylabel('Result')
    plt.show()

    g = sns.FacetGrid(dataframe, col='generation_method', hue='trial_status', col_wrap=4, height=4)
    g.map(sns.scatterplot, 'trial_index', 'result')
    g.add_legend()
    g.set_titles("{col_name}")
    g.set_axis_labels("Trial Index", "Result")
    plt.subplots_adjust(top=0.9)
    g.fig.suptitle('Results by Generation Method and Trial Status')
    plt.show()

    plt.figure(figsize=(10, 6))
    pivot_table = dataframe.pivot_table(index='generation_method', columns='trial_status', values='result', aggfunc='mean')
    sns.heatmap(pivot_table, annot=True, cmap='viridis')
    plt.title('Heatmap of Results by Generation Method and Trial Status')
    plt.xlabel('Trial Status')
