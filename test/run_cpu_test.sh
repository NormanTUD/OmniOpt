...
cd projects
perl cleanprojects.pl ../test/projects/cpu_test >/dev/null 2>/dev/null
cd -
...
export SBATCH_RESULT=$(sbatch -J cpu_test \
        --cpus-per-task=1 \
        --ntasks=1 \
        --time=00:20:00 \
        --mem-per-cpu=2000 \
        --partition=haswell \
        -o $THISLOGPATH \
        sbatch.pl --project=cpu_test --projectdir=test/projects/)
