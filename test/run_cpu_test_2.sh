...
cd projects
perl cleanprojects.pl ../test/projects/cpu_test2 >/dev/null 2>/dev/null
cd -
...
if ! sinfo --noheader -p haswell | grep -v resv 2>/dev/null >/dev/null; then
        echo "Haswell partition has no available nodes, not doing this test"
        exit 1
fi

export SBATCH_RESULT=$(sbatch -J cpu_test2 \
        --cpus-per-task=1 \
        --ntasks=1 \
        --time=00:20:00 \
        --mem-per-cpu=2000 \
        --partition=haswell \
        -o $THISLOGPATH \
        sbatch.pl --project=cpu_test2 --projectdir=test/projects/)
